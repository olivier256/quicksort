...
void quickSortTailCall(int *, int, int);
...
void quickSort(int *t, int left, int right) {
	if(right > left) {
		...
		quickSort(t, partitionPoint + 1, right);
	}		  
}

void quickSortTailCall(int *t, int left, int right) {
	if(right > left) {
		while (right > left) {
			int pivot = t[right];
			int partitionPoint = partition(t, left, right, pivot);
